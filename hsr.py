...
if is_ros2 is False:
    ...
    from OgnROS1ActionFollowJointTrajectory import InternalState as semuInternalState
    from OgnROS1ActionGripperCommand import InternalState as semuGripperInternalState
...
class VehicleDynamics:
    ...
    def forward(self, joint_space: JointSpace, state: VehicleState) -> CartSpace:
        ...
        output = CartSpace()
        output.dot_x = (
            (self._wheel_radius / 2.0 * cos_s - self._wheel_radius * self._wheel_offset / self._wheel_separation * sin_s) * joint_space.vel_wheel_r
            + (self._wheel_radius / 2.0 * cos_s + self._wheel_radius * self._wheel_offset / self._wheel_separation * sin_s) * joint_space.vel_wheel_l
        )
        output.dot_y = (
            (self._wheel_radius / 2.0 * sin_s + self._wheel_radius * self._wheel_offset / self._wheel_separation * cos_s) * joint_space.vel_wheel_r
            + (self._wheel_radius / 2.0 * sin_s - self._wheel_radius * self._wheel_offset / self._wheel_separation * cos_s) * joint_space.vel_wheel_l
        )
        output.dot_r = (
    ...
    def inverse(self, cart_space: CartSpace, state: VehicleState) -> JointSpace:
        ...
        output = JointSpace()
        output.vel_wheel_r = (
            (cos_s / self._wheel_radius - self._wheel_separation * sin_s / 2.0 / self._wheel_radius / self._wheel_offset) * cart_space.dot_x
            + (sin_s / self._wheel_radius + self._wheel_separation * cos_s / 2.0 / self._wheel_radius / self._wheel_offset) * cart_space.dot_y
        )
        output.vel_wheel_l = (
            (cos_s / self._wheel_radius + self._wheel_separation * sin_s / 2.0 / self._wheel_radius / self._wheel_offset) * cart_space.dot_x
            + (sin_s / self._wheel_radius - self._wheel_separation * cos_s / 2.0 / self._wheel_radius / self._wheel_offset) * cart_space.dot_y
...
class hsr:
    def __init__(self, prefix="/hsrb", stage_path="/World", config=None) -> None:
        ...
        self.hsr = stage.add_reference_to_stage(
            os.path.dirname(os.path.abspath(__file__)) + "/usd/hsrb/hsrb4s.usd",
            ...
            def init_action_server(srv, name, msg):
                ...
                if msg == FollowJointTrajectoryAction:
                        ...
                        if srv._action_goal is not None:
                            ...
                            if goal.trajectory.points[0].time_from_start.to_sec():
                                initial_point = JointTrajectoryPoint(
                                    positions=[srv._get_joint_position(name) for name in goal.trajectory.joint_names],
                    ...
                    srv.topic_interface = rospy.Subscriber(
                        action_topic_name.replace("/follow_joint_trajectory", "/command"),
                        JointTrajectory,
    ...
    def create_cameras(self) -> None:
        ...
        l_camera_prim = UsdGeom.Camera(
            omni.usd.get_context().get_stage().DefinePrim(self.stage_path + self.prefix + "/head_l_stereo_camera_link/Camera", "Camera")
        )
        ...
        r_camera_prim = UsdGeom.Camera(
            omni.usd.get_context().get_stage().DefinePrim(self.stage_path + self.prefix + "/head_r_stereo_camera_link/Camera", "Camera")
        )
        ...
        rgbd_camera_prim = UsdGeom.Camera(
            omni.usd.get_context().get_stage().DefinePrim(self.stage_path + self.prefix + "/head_rgbd_sensor_link/Camera", "Camera")
        )
        ...
        hand_camera_prim = UsdGeom.Camera(
            omni.usd.get_context().get_stage().DefinePrim(self.stage_path + self.prefix + "/hand_camera_frame/Camera", "Camera")
        )
            ...
            og.Controller.edit(
                {"graph_path": "/ros_controllers", "evaluator_name": "execution"},
                {
                    og.Controller.Keys.CREATE_NODES: [
                        ...
                        ("PublishClock", og_ros_node("isaacsim.ros1.bridge.ROS1PublishClock")),
                    ],
            ...
            (self.ros_camera_graph_l, _, _, _) = og.Controller.edit(
                ...
                },
                {
                    og.Controller.Keys.CREATE_NODES: [
                        ...
                        ("cameraHelperRgb", og_ros_node("isaacsim.ros1.bridge.ROS1CameraHelper")),
                        ("cameraHelperInfo", og_ros_node("isaacsim.ros1.bridge.ROS1CameraHelper")),
                    ],
                    og.Controller.Keys.CONNECT: [
            ...
            (self.ros_camera_graph_rgbd, _, _, _) = og.Controller.edit(
                ...
                },
                {
                    og.Controller.Keys.CREATE_NODES: [
                        ...
                        ("cameraHelperRgb", og_ros_node("isaacsim.ros1.bridge.ROS1CameraHelper")),
                        ("cameraHelperInfo", og_ros_node("isaacsim.ros1.bridge.ROS1CameraHelper")),
                        ("cameraHelperDepth", og_ros_node("isaacsim.ros1.bridge.ROS1CameraHelper")),
                        ("cameraHelperDepthInfo", og_ros_node("isaacsim.ros1.bridge.ROS1CameraHelper")),
                    ],
                    og.Controller.Keys.CONNECT: [
                        ("OnTick.outputs:tick", "createRenderProduct.inputs:execIn"),
                        ("createRenderProduct.outputs:execOut", "cameraHelperRgb.inputs:execIn"),
        ...
        (self.ros_lidar, _, _, _) = og.Controller.edit(
            ...
            },
            {
                og.Controller.Keys.CREATE_NODES: [
                    ...
                    ("publishLaserScan", og_ros_node("isaacsim.ros1.bridge.ROS1PublishLaserScan")),
                ],
                og.Controller.Keys.CONNECT: [
    ...
    def _create_controller_parameter_services(self) -> None:
        ...
        self._arm_get_parameters_srv = self.ros2node.create_service(
            ...
            lambda request, context: self._build_parameters_response(request, "joints", self._arm_controller_joints),
        )
        self._head_get_parameters_srv = self.ros2node.create_service(
            GetParameters,
            "/head_trajectory_controller/get_parameters",
            lambda request, context: self._build_parameters_response(request, "joints", self._head_controller_joints),
        )
        self._base_get_parameters_srv = self.ros2node.create_service(
            GetParameters,
            "/omni_base_controller/get_parameters",
            lambda request, context: self._build_parameters_response(request, "base_coordinates", self._base_controller_coordinates),
        )
        self._gripper_get_parameters_srv = self.ros2node.create_service(
            GetParameters,
            "/gripper_controller/get_parameters",
            lambda request, context: self._build_parameters_response(request, "joints", self._gripper_controller_joints),
        )
    ...
    def set_base_joint_and_material(self) -> None:
        ...
        left_passive1_drive = UsdPhysics.DriveAPI.Get(
            stage.get_current_stage().GetPrimAtPath(self.stage_path + self.prefix + "/base_l_passive_wheel_x_frame/base_l_passive_wheel_y_frame_joint"),
            "angular",
        )
        ...
        left_passive2_drive = UsdPhysics.DriveAPI.Get(
            stage.get_current_stage().GetPrimAtPath(self.stage_path + self.prefix + "/base_l_passive_wheel_y_frame/base_l_passive_wheel_z_joint"),
            "angular",
        )
        ...
        right_passive1_drive = UsdPhysics.DriveAPI.Get(
            stage.get_current_stage().GetPrimAtPath(self.stage_path + self.prefix + "/base_r_passive_wheel_x_frame/base_r_passive_wheel_y_frame_joint"),
            "angular",
        )
        ...
        right_passive2_drive = UsdPhysics.DriveAPI.Get(
            stage.get_current_stage().GetPrimAtPath(self.stage_path + self.prefix + "/base_r_passive_wheel_y_frame/base_r_passive_wheel_z_joint"),
            "angular",
        )
        ...
        left_wheel_drive = UsdPhysics.DriveAPI.Get(
            stage.get_current_stage().GetPrimAtPath(self.stage_path + self.prefix + "/base_roll_link/base_l_drive_wheel_joint"),
            "angular",
        )
        right_wheel_drive = UsdPhysics.DriveAPI.Get(
            stage.get_current_stage().GetPrimAtPath(self.stage_path + self.prefix + "/base_roll_link/base_r_drive_wheel_joint"),
            "angular",
        )
        roll_drive = UsdPhysics.DriveAPI.Get(
            stage.get_current_stage().GetPrimAtPath(self.stage_path + self.prefix + "/base_link/base_roll_joint"),
            "angular",
        )
    ...
    def step(self):
            ...
            for i in range(self.dc.get_articulation_dof_count(self.art)):
                ...
                if dof_ptr != _dynamic_control.DofType.DOF_NONE:
                    ...
                    self._joints[dof_name] = (
                        ...
                        dof_name in ["arm_flex_joint", "arm_lift_joint", "wrist_flex_joint", "arm_roll_joint"],
                    )
            print(self._joints)
        ...
        force_readings = self.robots.get_measured_joint_forces(
            joint_indices=[self.robots._metadata.joint_indices["wrist_ft_sensor_frame_joint"] + 1]
        )
